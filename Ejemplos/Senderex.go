...
type ChangeValue struct {
	...
	Statuses         []Status   `json:"statuses"` // Ignoramos status updates por ahora
}
...
type Message struct {
	From      string   `json:"from"`      // Número del remitente en formato E.164
	ID        string   `json:"id"`        // messageId único — usado para deduplicación
...
type Sender struct {
	...
	apiVersion    string       // WHATSAPP_API_VERSION (default: v22.0)
...
// SendText envía un mensaje de texto plano al número destinatario.
// to debe estar en formato E.164 (ej: "50686091954").
