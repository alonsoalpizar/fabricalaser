...
// Nunca acepta credenciales hardcodeadas.
...
func (h *Handler) HandleMessage(w http.ResponseWriter, r *http.Request) {
	...
	// El procesamiento real ocurre en goroutine separada
	w.WriteHeader(http.StatusOK)
	w.Write([]byte(`{"status":"ok"}`))

	// 5. Procesar mensaje de forma asíncrona
	go h.processor.Process(r.Context(), &payload)
}

// verifySignature valida la firma HMAC-SHA256 que Meta adjunta en cada request.
// Protege el webhook de requests externos maliciosos.
